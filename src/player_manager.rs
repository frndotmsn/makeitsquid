...
async fn select_new_meme_template(database_connection_pool: &PgPool, player_id: i32) -> anyhow::Result<Option<i32>> {
    ...
    // we do not need to specify the type of "main_query.id"
    // just to say it's forced not-null
    ...
    let new_meme_template_id: Option<i32> = sqlx::query_scalar!(
        ...
        ORDER BY RANDOM() LIMIT 1"#, player_id)
...
impl PlayerManager {
    ...
    pub async fn sign_in_as_guest(&self, name: String) -> anyhow::Result<()> {
        let player_id: i32 = sqlx::query_scalar!(r#"INSERT INTO players (name) VALUES ($1) RETURNING id as "id!""#, name)
    ...
    pub async fn get_meme_template(&self, meme_template_id: i32) -> anyhow::Result<Option<MemeTemplate>> {
        let meme_template: Option<MemeTemplate> = sqlx::query_as!(
                MemeTemplate,
                "SELECT * FROM meme_templates WHERE id = $1",
                meme_template_id)
    ...
    pub async fn get_meme_template_id_for_player(&self, user_id: i32) -> anyhow::Result<Option<i32>> {
        let id: Option<i32> = sqlx::query_scalar!(
            r#"SELECT selected_meme_template_id as "selected_meme_template_id?" FROM players WHERE id = $1"#,
            user_id)
    ...
    pub async fn get_remaining_rerolls(&self, player_id: i32) -> anyhow::Result<i32> {
        let rerolls_left: i32 = sqlx::query_scalar!(r#"SELECT rerolls_left as "rerolls_left!" FROM players WHERE id = $1"#, player_id)
            .fetch_one(&self.database_connection_pool)
            .await?;
